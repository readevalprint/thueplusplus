...
describe('Go-WASM demo UI', () => {
  ...
  it('serves the challenges index route with pilot challenge links', async () => {
    ...
    const contributeLinks = wrapper.get('[data-test="challenge-index-contribute-links"]')
    expect(contributeLinks.text()).toContain('Contribute:')
    expect(contributeLinks.find('a[href="https://gitlab.com/thuelang/thueplusplus/-/blob/develop/challenges/CONTRIBUTING.md#challenges"]').exists()).toBe(true)
    expect(contributeLinks.find('a[href="https://gitlab.com/thuelang/thueplusplus/-/blob/develop/challenges/CONTRIBUTING.md#test-cases"]').exists()).toBe(true)
    expect(contributeLinks.find('a[href="https://gitlab.com/thuelang/thueplusplus/-/blob/develop/challenges/CONTRIBUTING.md#solutions"]').exists()).toBe(true)
  ...
  it('runs challenge tests from the playground rules editor and shows expected output diffs', async () => {
    ...
    const challengeContributeLinks = wrapper.get('[data-test="challenge-contribute-links"]')
    expect(challengeContributeLinks.text()).toContain('Suggest improvements:')
    expect(challengeContributeLinks.find('a[href="https://gitlab.com/thuelang/thueplusplus/-/blob/develop/challenges/CONTRIBUTING.md#challenges"]').exists()).toBe(true)
    expect(challengeContributeLinks.find('a[href="https://gitlab.com/thuelang/thueplusplus/-/blob/develop/challenges/CONTRIBUTING.md#test-cases"]').exists()).toBe(true)
    expect(challengeContributeLinks.find('a[href="https://gitlab.com/thuelang/thueplusplus/-/blob/develop/challenges/CONTRIBUTING.md#solutions"]').exists()).toBe(true)
    ...
    expect(wrapper.find('[data-test="challenge-attempt-metrics"]').exists()).toBe(false)
  ...
  it('runs challenge tests visibly through playground play controls when auto is off', async () => {
    ...
    try {
      ...
      expect(wrapper.get('[data-test="challenge-publish-cta"]').attributes('href')).toBe('https://gitlab.com/thuelang/thueplusplus/-/blob/develop/challenges/CONTRIBUTING.md#solutions')
  ...
  it('discovers resources from hash-prefixed rule rows', async () => {
    window.history.pushState({}, '', '/playground?file=./examples/hello/hello.tpp')
    const wrapper = await mountApp()
    await wrapper.get('[data-test="playground-rules"]').setValue('#read ::< 1s 1 lines custom\n#done ::> custom done\n')
  ...
  it('keeps visible resource input stable while history carries copied read cursors', async () => {
    ...
    const wrapper = await mountApp({ attachTo: document.body })
    await wrapper.get('[data-test="playground-rules"]').setValue('@IN@ ::< 30s 1 lines stdin\n^Ada$ ::= done')
  ...
  it('submits an empty line when a millisecond pending input countdown expires', async () => {
    ...
    try {
      window.history.pushState({}, '', '/playground?file=./examples/echo/echo.tpp')
      const wrapper = await mountApp({ attachTo: document.body })
      await wrapper.get('[data-test="playground-rules"]').setValue('@IN@ ::< 500ms 1 lines stdin')
      await setProgramState(wrapper, '@IN@')
  ...
  it('shows minute read timeout countdowns in seconds', async () => {
    ...
    try {
      window.history.pushState({}, '', '/playground?file=./examples/echo/echo.tpp')
      const wrapper = await mountApp({ attachTo: document.body })
      await wrapper.get('[data-test="playground-rules"]').setValue('@IN@ ::< 1m 1 lines stdin')
      await setProgramState(wrapper, '@IN@')
  ...
  it('clears a pending countdown when typed resource input changes', async () => {
    ...
    try {
      window.history.pushState({}, '', '/playground?file=./examples/echo/echo.tpp')
      const wrapper = await mountApp({ attachTo: document.body })
      await wrapper.get('[data-test="playground-rules"]').setValue('@IN@ ::< 1s 1 lines stdin')
      await setProgramState(wrapper, '@IN@')
