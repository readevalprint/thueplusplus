...
describe('Go-WASM demo UI', () => {
  ...
  it('mounts the app and runs the hello stdout example', async () => {
    const wrapper = mount(App)

    expect(wrapper.text()).toContain('thue++ Go-WASM demo')
    expect(wrapper.text()).toContain('Hello stdout')
    ...
    expect(wrapper.text()).toContain('Exit code0')
  ...
  it('sends custom callback resources and displays read/write logs', async () => {
    const wrapper = mount(App)
  ...
  it('renders raw coverage TSV and parsed coverage rows', async () => {
    const wrapper = mount(App)
