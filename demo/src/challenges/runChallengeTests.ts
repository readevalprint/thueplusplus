// SPDX-License-Identifier: AGPL-3.0-or-later
import { runWithWorker, type DemoResourceLog } from '../wasm'
import type { ChallengeAttemptMetrics, ChallengeEntry, ChallengeTestCase } from './types'

export interface ChallengeResourceResult {
  name: string
  expected: string
  actual: string
  passed: boolean
}

export interface ChallengeExitCodeResult {
  expected: number
  actual: number | undefined
  passed: boolean
}

export interface ChallengeTestResult {
  name: string
  passed: boolean
  exitCode: ChallengeExitCodeResult
  resources: ChallengeResourceResult[]
  metrics?: ChallengeAttemptMetrics
  error?: string
}

export async function runChallengeTests(challenge: ChallengeEntry, source: string, signal?: AbortSignal, fallbackInput = ''): Promise<ChallengeTestResult[]> {
  const results: ChallengeTestResult[] = []
  for (const testCase of challenge.tests) {
    if (signal?.aborted) throw abortError()
    const result = await runChallengeTest(challenge, source, testCase, signal, fallbackInput)
    results.push(result)
    if (!result.passed) break
  }
  return results
}

async function runChallengeTest(challenge: ChallengeEntry, source: string, testCase: ChallengeTestCase, signal?: AbortSignal, fallbackInput = ''): Promise<ChallengeTestResult> {
  try {
    const result = await runWithWorker({
      sourceText: source,
      sourcePath: `challenges/${challenge.slug}/attempt.tpp`,
      input: testCase.resources.stdin?.buffer ?? fallbackInput,
      evalLimit: 10000,
      maxStateBytes: 1_000_000,
      coverage: false,
      resources: Object.entries(testCase.resources)
        .filter(([name]) => name !== 'stdin' && name !== 'stdout' && name !== 'stderr')
        .map(([name, resource]) => ({
          name,
          inputText: resource.buffer ?? '',
          lineMode: true,
        })),
      signal,
    })
    if (typeof result.exitCode !== 'number') {
      throw new Error('worker did not return exitCode')
    }
    const actualExitCode = result.exitCode
    const exitCode = {
      expected: testCase.exit_code,
      actual: actualExitCode,
      passed: actualExitCode === testCase.exit_code,
    }
    const resources = expectedResources(testCase).map(({ name, expected }) => {
      const actual = normalizeOutput(resourceOutput(result.resourceLogs, name, result.stdout ?? '', result.stderr ?? ''))
      const normalizedExpected = normalizeOutput(expected)
      return {
        name,
        expected: normalizedExpected,
        actual,
        passed: actual === normalizedExpected,
      }
    })
    return {
      name: testCase.name,
      passed: exitCode.passed && resources.every(resource => resource.passed) && !result.error,
      exitCode,
      resources,
      error: result.error || result.errors || undefined,
    }
  } catch (error) {
    if (isAbortError(error)) throw error
    return {
      name: testCase.name,
      passed: false,
      exitCode: {
        expected: testCase.exit_code,
        actual: undefined,
        passed: false,
      },
      resources: expectedResources(testCase).map(({ name, expected }) => ({
        name,
        expected: normalizeOutput(expected),
        actual: '',
        passed: false,
      })),
      error: error instanceof Error ? error.message : String(error),
    }
  }
}

export function expectedResources(testCase: ChallengeTestCase): Array<{ name: string; expected: string }> {
  return Object.entries(testCase.resources)
    .filter((entry): entry is [string, { expected_output: string }] => typeof entry[1].expected_output === 'string')
    .map(([name, resource]) => ({ name, expected: resource.expected_output }))
}

function resourceOutput(logs: DemoResourceLog[] | undefined, name: string, stdout: string, stderr: string): string {
  const log = logs?.find(resource => resource.name === name)
  if (log) return log.outputText ?? log.writes.join('')
  if (name === 'stdout') return stdout
  if (name === 'stderr') return stderr
  return ''
}

function normalizeOutput(output: string): string {
  return output.trim()
}

function abortError(): DOMException {
  return new DOMException('Challenge test run aborted', 'AbortError')
}

function isAbortError(error: unknown): boolean {
  return error instanceof DOMException && error.name === 'AbortError'
}
