...
test('timeline click restores state and step prunes future rows', async ({ page }) => {
  ...
  await expect(page.locator('.state-diff-entry')).toHaveCount(3)
  await expect(page.getByTestId('playground-status')).toContainText('exited 0')
  await expect(page.locator('.state-diff-entry').nth(0)).toContainText('initial state')
  await expect(page.locator('.state-diff-entry').nth(1)).toContainText('^start$ ::= middle')
  await expect(page.locator('.state-diff-entry').nth(2)).toContainText('^middle$ ::= done')
  ...
  await page.locator('.state-diff-entry').nth(1).click()
