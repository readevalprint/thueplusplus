...
# Attempt Z: dynamic scoped lambda-apply helper. This is intentionally a narrow closure/apply
# probe: the generated rule is named with an explicit id and computes the body for one
# 3-argument lambda-shaped call. It demonstrates scoped generated apply, not full closures.
^RAWLAM:(?<id>[A-Za-z0-9_]+):(?<a>-?[0-9]+),(?<b>-?[0-9]+),(?<c>-?[0-9]+)$ ::= ^APPLY_LAM3_{{id}}$ ::= C2ADDAB[ADD[{{a}},{{b}}],{{c}}|E0|KDONE]\nAPPLY_LAM3_{{id}}

# Attempt Zb: dynamic curried surface frame into the first-class-ish C0/C1/C2 call path.
^RAWCUR:(?<id>[A-Za-z0-9_]+):(?<a>-?[0-9]+),(?<b>-?[0-9]+),(?<c>-?[0-9]+)$ ::= ^PARSE_CUR_{{id}}$ ::= EV[L%3AL%253AL%25253AA%2525253Alam3cur%25252520N%2525253A{{a}}%25252520%252520N%25253A{{b}}%252520%2520N%253A{{c}}%2520%20|E0|KDONE]\nPARSE_CUR_{{id}}
