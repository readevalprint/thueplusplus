...
^EVLIST\[A%3A(?<op>add|mul|eq)%20(?<left>L%3A<|PCT|>%20)(?<right><|ITEM|>)\|(?<k><|PCT|>)\]$ ::= EV[{{left}}|KLEFT%28{{op}}%7C{{right|pctenc}}%7C{{k}}%29]
^RET\[(?<v><|VAL|>)%20\|KLEFT%28(?<op>add|mul|eq)%7C(?<right><|PCT|>)%7C(?<k><|PCT|>)%29\]$ ::= EVLIST[A%3A{{op}}%20{{v}}%20{{right|pctdec}}|{{k}}]
^EVLIST\[A%3A(?<op>add|mul|eq)%20(?<left><|VAL|>)%20(?<right>L%3A<|PCT|>%20)\|(?<k><|PCT|>)\]$ ::= EV[{{right}}|KRIGHT%28{{op}}%7C{{left}}%7C{{k}}%29]
^RET\[(?<v><|VAL|>)%20\|KRIGHT%28(?<op>add|mul|eq)%7C(?<left><|VAL|>)%7C(?<k><|PCT|>)%29\]$ ::= EVLIST[A%3A{{op}}%20{{left}}%20{{v}}%20|{{k}}]
