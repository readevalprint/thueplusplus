...
# Env-aware array selectors must be special forms, not generic calls to variable `rest`/`head`.
...
# Let single-binding as sugar over env extension.
^E\[let L%5BL%255B(?<n1>[A-Za-z_][A-Za-z0-9_-]*)%2520(?<v1><|PCT|>)%255D%20L%255B(?<n2>[A-Za-z_][A-Za-z0-9_-]*)%2520(?<v2><|PCT|>)%255D%5D (?<body><|NODE|>)\|(?<k>.*)\]$ ::= LETARG[{{v1|pctdec}}|KLET2A[{{n1}}|{{n2}}|{{v2}}|{{body}}] {{k}}]
^RET\[(?<v><|VAL|>)\|KLET2A\[(?<n1>[A-Za-z_][A-Za-z0-9_-]*)\|(?<n2>[A-Za-z_][A-Za-z0-9_-]*)\|(?<v2><|PCT|>)\|(?<body><|NODE|>)\] (?<k>.*)\]$ ::= LETARG[{{v2|pctdec}}|KLET2B[{{n2}}|{{body}}|{{n1}}={{v|pctenc}};] {{k}}]
^RET\[(?<v><|VAL|>)\|KLET2B\[(?<n2>[A-Za-z_][A-Za-z0-9_-]*)\|(?<body><|NODE|>)\|(?<env>[^|]*)\] (?<k>.*)\]$ ::= EENV[{{body}}|{{n2}}={{v|pctenc}};{{env}}|{{k}}]
^E\[let L%5BL%255B(?<n>[A-Za-z_][A-Za-z0-9_-]*)%2520(?<v><|PCT|>)%255D%5D (?<body><|NODE|>)\|(?<k>.*)\]$ ::= LETARG[{{v|pctdec}}|KLET[{{n}}|{{body}}] {{k}}]
^LETARG\[(?<node><|PCT|>)\|(?<k>.*)\]$ ::= ARG[{{node|pctdec}}|{{k}}]
^LETARG\[(?<node>[^|]*)\|(?<k>.*)\]$ ::= ARG[{{node}}|{{k}}]
^RET\[(?<v><|VAL|>)\|KLET\[(?<n>[A-Za-z_][A-Za-z0-9_-]*)\|(?<body><|NODE|>)\] (?<k>.*)\]$ ::= EENV[{{body}}|{{n}}={{v|pctenc}};|{{k}}]
