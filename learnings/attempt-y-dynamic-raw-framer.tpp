...
# Attempt Y: dynamic raw-input framer. These rules generate an exact one-shot PARSE_* rule
# plus a matching PARSE_* row. The generated rule emits the existing item-stream format.
# It is deliberately not a full parser; it proves dynamic rule generation can be used as
# a parser/framer front-end for arithmetic, compare, and lazy booleans.
^RAW:add5:(?<a>-?[0-9]+),(?<b>-?[0-9]+),(?<c>-?[0-9]+),(?<d>-?[0-9]+),(?<e>-?[0-9]+)$ ::= ^PARSE_ADD5_{{a}}_{{b}}_{{c}}_{{d}}_{{e}}$ ::= EVLIST[A%3Aadd%20N%3A{{a}}%20N%3A{{b}}%20N%3A{{c}}%20N%3A{{d}}%20N%3A{{e}}%20|E0|KDONE]\nPARSE_ADD5_{{a}}_{{b}}_{{c}}_{{d}}_{{e}}
^RAW:muladd:(?<a>-?[0-9]+),(?<b>-?[0-9]+),(?<c>-?[0-9]+),(?<d>-?[0-9]+)$ ::= ^PARSE_MULADD_{{a}}_{{b}}_{{c}}_{{d}}$ ::= EVLIST[A%3Amul%20L%3AA%253Aadd%2520N%253A{{a}}%2520N%253A{{b}}%2520%20L%3AA%253Aadd%2520N%253A{{c}}%2520N%253A{{d}}%2520%20|E0|KDONE]\nPARSE_MULADD_{{a}}_{{b}}_{{c}}_{{d}}
^RAW:eqadd:(?<a>-?[0-9]+),(?<b>-?[0-9]+),(?<c>-?[0-9]+)$ ::= ^PARSE_EQADD_{{a}}_{{b}}_{{c}}$ ::= EVLIST[A%3Aeq%20L%3AA%253Aadd%2520N%253A{{a}}%2520N%253A{{b}}%2520%20N%3A{{c}}%20|E0|KDONE]\nPARSE_EQADD_{{a}}_{{b}}_{{c}}
^RAW:iftrue:(?<t>-?[0-9]+)$ ::= ^PARSE_IFTRUE_{{t}}$ ::= EVLIST[A%3Aif%20B%3Atrue%20N%3A{{t}}%20L%3AA%253Adiv%2520N%253A1%2520N%253A0%2520%20|E0|KDONE]\nPARSE_IFTRUE_{{t}}
^RAW:andfalse$ ::= ^PARSE_ANDFALSE$ ::= EVLIST[A%3Aand%20B%3Afalse%20L%3AA%253Adiv%2520N%253A1%2520N%253A0%2520%20|E0|KDONE]\nPARSE_ANDFALSE
