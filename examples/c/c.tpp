...
^SEMA<TU<FN<RET<int>\|NAME<(?<name>$PCT)>\|PARAMS<>\|BODY<(?<body>[\s\S]+)>>>>$ ::= SEMA<TU<FN<RET<int>|NAME<{{name}}>|PARAMS<void>|BODY<{{body}}>>>>
...
^SEMA<TU<FN<RET<int>\|NAME<(?<name>$PCT)>\|PARAMS<void>\|BODY<RETURN<ID<(?<id>$PCT)>>>>>$ ::= ERR<undefined_identifier>
...
# state records.
^EXEC<TU<SCOPE<file\|BIND<(?<bind>$PCT)\|function\|FUNC<int\|void>>>\|FN<TYPE<FUNC<int\|void>>\|NAME<(?<name>$PCT)>\|BODY<RETURN<RVAL<int\|(?<n>$PCT)>>>>>>$ ::= EXEC_MAIN<{{bind}}|{{name}}|RETURN<RVAL<int|{{n}}>>|@EQ[{{bind}}|{{name}}]@>
^EXEC<TU<SCOPE<file\|BIND<(?<bind>$PCT)\|function\|FUNC<int\|void>>>\|FN<TYPE<FUNC<int\|void>>\|NAME<(?<name>$PCT)>\|BODY<COMPOUND<DECL<LVAL<int\|(?<local>$PCT)>>\|RETURN<RVAL<int\|(?<n>$PCT)>>>>>>$ ::= EXEC_MAIN<{{bind}}|{{name}}|COMPOUND<DECL<LVAL<int|{{local}}>>|RETURN<RVAL<int|{{n}}>>>|@EQ[{{bind}}|{{name}}]@>
^EXEC<TU<SCOPE<file\|BIND<(?<bind>$PCT)\|function\|FUNC<int\|void>>>\|FN<TYPE<FUNC<int\|void>>\|NAME<(?<name>$PCT)>\|BODY<COMPOUND<DECL_INIT<LVAL<int\|(?<local>$PCT)>\|RVAL<int\|(?<n>$PCT)>>\|RETURN<RVAL<int\|LOAD<LVAL<int\|(?<retid>$PCT)>>>>>>>>$ ::= EXEC_MAIN<{{bind}}|{{name}}|COMPOUND<DECL_INIT<LVAL<int|{{local}}>|RVAL<int|{{n}}>>|RETURN<RVAL<int|LOAD<LVAL<int|{{retid}}>>>>>|@EQ[{{bind}}|{{name}}]@>
^EXEC<TU<SCOPE<file\|BIND<(?<bind>$PCT)\|function\|FUNC<int\|void>>>\|FN<TYPE<FUNC<int\|void>>\|NAME<(?<name>$PCT)>\|BODY<COMPOUND<DECL<LVAL<int\|(?<local>$PCT)>>\|ASSIGN<LVAL<int\|(?<lhs>$PCT)>\|RVAL<int\|(?<n>$PCT)>>\|RETURN<RVAL<int\|LOAD<LVAL<int\|(?<retid>$PCT)>>>>>>>>$ ::= EXEC_MAIN<{{bind}}|{{name}}|COMPOUND<DECL<LVAL<int|{{local}}>>|ASSIGN<LVAL<int|{{lhs}}>|RVAL<int|{{n}}>>|RETURN<RVAL<int|LOAD<LVAL<int|{{retid}}>>>>>|@EQ[{{bind}}|{{name}}]@>
^EXEC<TU<SCOPE<file\|BIND<(?<bind>$PCT)\|function\|FUNC<int\|void>>>\|FN<TYPE<FUNC<int\|void>>\|NAME<(?<name>$PCT)>\|BODY<IF<RVAL<int\|(?<cond>$PCT)>\|RETURN<RVAL<int\|(?<then>$PCT)>>\|RETURN<RVAL<int\|(?<elsev>$PCT)>>>>>>$ ::= EXEC_MAIN<{{bind}}|{{name}}|IF<RVAL<int|{{cond}}>|RETURN<RVAL<int|{{then}}>>|RETURN<RVAL<int|{{elsev}}>>>|@EQ[{{bind}}|{{name}}]@>
^EXEC_MAIN<(?<bind>$PCT)\|(?<name>$PCT)\|(?<body>[\s\S]+)\|1>$ ::= EXEC_BODY<{{body}}>
^EXEC_MAIN<(?<bind>$PCT)\|(?<name>$PCT)\|(?<body>[\s\S]+)\|0>$ ::= ERR<unsupported_c_construct>
^EXEC_BODY<RETURN<RVAL<int\|(?<n>$PCT)>>>$ ::= @OUT<{{n}}>
^EXEC_BODY<COMPOUND<DECL<LVAL<int\|(?<local>$PCT)>>\|RETURN<RVAL<int\|(?<n>$PCT)>>>>$ ::= EXEC_BODY<RETURN<RVAL<int|{{n}}>>>
^EXEC_BODY<COMPOUND<DECL_INIT<LVAL<int\|(?<local>$PCT)>\|RVAL<int\|(?<n>$PCT)>>\|RETURN<RVAL<int\|LOAD<LVAL<int\|(?<retid>$PCT)>>>>>>$ ::= EXEC_LOCAL_RETURN<{{local}}|{{retid}}|{{n}}|@EQ[{{local}}|{{retid}}]@>
^EXEC_BODY<COMPOUND<DECL<LVAL<int\|(?<local>$PCT)>>\|ASSIGN<LVAL<int\|(?<lhs>$PCT)>\|RVAL<int\|(?<n>$PCT)>>\|RETURN<RVAL<int\|LOAD<LVAL<int\|(?<retid>$PCT)>>>>>>$ ::= EXEC_LOCAL_ASSIGN_RETURN<{{local}}|{{lhs}}|{{retid}}|{{n}}|@EQ[{{local}}|{{lhs}}]@|@EQ[{{local}}|{{retid}}]@>
^EXEC_BODY<IF<RVAL<int\|1>\|RETURN<RVAL<int\|(?<then>$PCT)>>\|RETURN<RVAL<int\|(?<elsev>$PCT)>>>>$ ::= EXEC_BODY<RETURN<RVAL<int|{{then}}>>>
^EXEC_BODY<IF<RVAL<int\|0>\|RETURN<RVAL<int\|(?<then>$PCT)>>\|RETURN<RVAL<int\|(?<elsev>$PCT)>>>>$ ::= EXEC_BODY<RETURN<RVAL<int|{{elsev}}>>>
^EXEC_BODY<IF<RVAL<int\|(?<cond>$PCT)>\|RETURN<RVAL<int\|(?<then>$PCT)>>\|RETURN<RVAL<int\|(?<elsev>$PCT)>>>>$ ::= ERR<unsupported_c_construct>
^EXEC_LOCAL_RETURN<(?<local>$PCT)\|(?<retid>$PCT)\|(?<n>$PCT)\|1>$ ::= @OUT<{{n}}>
^EXEC_LOCAL_RETURN<(?<local>$PCT)\|(?<retid>$PCT)\|(?<n>$PCT)\|0>$ ::= ERR<unsupported_c_construct>
^EXEC_LOCAL_ASSIGN_RETURN<(?<local>$PCT)\|(?<lhs>$PCT)\|(?<retid>$PCT)\|(?<n>$PCT)\|1\|1>$ ::= @OUT<{{n}}>
^EXEC_LOCAL_ASSIGN_RETURN<(?<local>$PCT)\|(?<lhs>$PCT)\|(?<retid>$PCT)\|(?<n>$PCT)\|(?<assignok>0|1)\|(?<retok>0|1)>$ ::= ERR<unsupported_c_construct>
