...
It also accepts a declaration followed by assigning and returning that same
local identifier:

```bash
uv run python python/thuepp.py examples/c/c.tpp --input 'int main(void) { int x; x = 9; return x; }'
```
...
It also accepts a constant-condition `if/else` that returns an integer from
both branches:

```bash
uv run python python/thuepp.py examples/c/c.tpp --input 'int main(void) { if (1) { return 7; } else { return 8; } }'
```
...
Any C construct outside this scaffold fails loudly. The raw smoke now has one conservative `int main(void)` / `int main()` body gate that admits tokenizable main bodies into the shared lexer and lets parser/sema/exec decide the narrower supported subset. It accepts bare integer constants (`ICON`: decimal or hexadecimal) and decimal arithmetic expression chains made from decimal integer operands plus `+` and `*`, not fractions, decimals, parentheses, division, subtraction, unary operators, hexadecimal arithmetic operands, or identifier operands. The local-declaration slice accepts `int <identifier>;` before an integer return, `int <identifier> = <ICON>; return <same-identifier>;`, and `int <identifier>; <same-identifier> = <ICON>; return <same-identifier>;`; multiple declarations, multiple assignments, non-constant initializers/assignments, `if` without `else`, non-constant `if` conditions, and returning or assigning a different local name are not yet source-driven. Tokenized-but-unsupported bodies fail at parser/sema with stable diagnostics such as `syntax_error`, `undefined_identifier`, or `unsupported_c_construct`; trailing source after the closing brace is still rejected before tokenization. Supported raw source reaches output through the lexer-backed public parser/sema/exec pipeline rather than a direct raw-source success rule; the source-pipeline parser canonicalizes both `main(void)` and `main()` to the same internal `PARAMS<void>` function type before semantic lowering. Downstream cards must extend the staged C pipeline described below, not add one raw regex per exact body shape.
...
- `int main(void) { int <identifier>; <same-identifier> = <number>; return <same-identifier>; }`
- `int main() { int <identifier>; <same-identifier> = <number>; return <same-identifier>; }`
- `int main(void) { if (<0-or-1>) { return <number>; } else { return <number>; } }`
- `int main() { if (<0-or-1>) { return <number>; } else { return <number>; } }`
