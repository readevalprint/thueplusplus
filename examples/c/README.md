...
- source-driven `main` definitions support a single declaration-only local `int` before an integer return, a single initialized local `int` before returning that same local identifier, and a single declaration plus same-local assignment before returning that local;
