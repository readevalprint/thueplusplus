...
- fully general lambda application and lexical closure evaluation; this slice now tests alpha-renamed `let`/`let*`/lambda lookup and rejects mismatched identifiers/arity/non-procedure application for the supported zero-, one-, and two-argument shapes, but still does not implement a complete datum-level evaluator
