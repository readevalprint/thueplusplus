...
test:
	...
	uv run --with pytest pytest tools/test_example_runner.py tools/test_submission_autoapprove.py -q --tb=short
...
challenge-test: challenge-check
	@command -v uv >/dev/null 2>&1 || { echo "Error: uv is required" >&2; exit 127; }
	uv run --with pytest pytest tools/test_challenge_generator.py tools/test_submission_autoapprove.py -q --tb=short
