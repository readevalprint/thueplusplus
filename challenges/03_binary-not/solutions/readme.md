# Binary Not Solutions

<!-- challenges:leaderboard:start -->
| Rank | Solution | Rules | Steps | Eval Checks | Cumulative State |
|---:|---|---:|---:|---:|---:|
| 1 | Truth Table Flip2 | 6 | 8 | 28 | 175 bytes |

### Best-In-Class Records

- Fewest Rules: Truth Table Flip2
- Lowest Steps: Truth Table Flip2
- Lowest Eval Checks: Truth Table Flip2
- Lowest Cumulative State per Step: Truth Table Flip2

_Only solutions that pass every case on the Go backend with 100% rule coverage are ranked._
<!-- challenges:leaderboard:end -->
